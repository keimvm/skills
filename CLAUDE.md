...
- 対話系スキル（natural-japanese, thinking-partner, to-spec, wait-what）は `disable-model-invocation: true` を付け、ユーザーが明示的に呼ぶ前提にしている。`html` だけは付けず、モデルが自動選択できる。新規スキルもこの区別に従う。
